...
3. 把 `kb-suite` 的 35 个 Skills 作为项目能力部署在知识库内，并让面板直接反映其信息架构和合同；
...
| `kb-workspace.js` | 六库运行模型、35 个 Skill 目录、内容流水线和七个主视图 |
...
模板业务结构来自 `kb-suite`：录音处理、老板说明书、产品库、客户需求库、素材案例库、方法论库、内容生产。蓝图 v3 包含 143 个目录和 500 个文件，其中包括完整的 35 个项目 Skills、suite 版本与能力清单。`7-系统` 是本插件增加的运行目录，只放 AI 运行记录和输出，不计入六类业务资产。

运行时由 `KBWorkspaceService` 重新读取 `.kb/config.yaml`，解析六类资产路径；路径配置不是只在构建期烘焙进 UI。正式资产统计明确排除 `.agents`、`.kb`、`7-系统` 和目录说明文件。
