...
for (const required of ["kb-start", "kb-interview", "kb-ingest", "kb-library", "kb-write", "kb-review", "kb-health"]) {
...
const workspaceSource = readFileSync(fileURLToPath(new URL("../kb-workspace.js", `file://${__filename}`)), "utf8");
...
assert.match(styles, /@container wis-view \(max-width:\s*1450px\)[\s\S]*?\.wis-today-signal\s*\{[\s\S]*?grid-template-columns:\s*28px minmax\(0,\s*1fr\)[\s\S]*?width:\s*min\(100%,\s*440px\)/, "dashboard signal must reflow before its kb-start action can escape the card");
...
console.log("PASS kb workspace uses six libraries, six content stages, 35 skills, unified AI modes, overflow-safe controls and source-aware quality rules");
