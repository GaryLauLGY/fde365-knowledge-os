...
class KBWorkspaceService {
  ...
  async reloadConfig() {
    const configPath = `${ROOT}/.kb/config.yaml`;
    try {
...
class KBBaseView extends ItemView {
  ...
  assistantContextFiles() {
    return this.assistantSourcePaths
  ...
  renderAssistantComposer(panel) {
    ...
    note.addEventListener("click", async () => {
      if (!this.isAssistantContextFile(active)) {
        new Notice("请先在FDE365知识库中打开一篇业务笔记");
        return;
      }
      this.assistantSourcePaths = activeAttached
        ? this.assistantSourcePaths.filter((path) => path !== active.path)
        : [...new Set([...this.assistantSourcePaths, active.path])];
      await this.render();
    });
...
class KBDashboardView extends KBBaseView {
...
class KBInboxView extends KBBaseView {
  ...
  async renderMain(main, data) {
    ...
    data.pending.forEach((file) => {
      ...
      makeButton(actions, "生成分流预览", "sparkles", "is-secondary", () => this.service.runSkill("kb-ingest", `请处理这份原始材料：${file.path}。先生成分流预览，不要直接写入正式资产。`, [file]));
    });
  }
}

class KBLibrariesView extends KBBaseView {
...
class KBNetworkView extends KBBaseView {
...
class KBContentView extends KBBaseView {
...
class KBHealthView extends KBBaseView {
  ...
  async renderMain(main, data) {
    ...
    [
      { level: data.missingPaths.length ? "block" : "ok", title: "六库路径", value: data.missingPaths.length ? `${data.missingPaths.length} 个缺失` : "配置与目录存在", note: data.missingPaths[0] || `${ROOT}/.kb/config.yaml` },
