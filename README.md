...
- **AI 语音快速记录**：在待处理页直接录音、暂停和结束，通过内置 FDE365 语音模型生成可编辑转写；原始音频与转写稿一起保存，由用户决定是否运行 `/fde-ingest`。
- **总览**：六库真实文件数、来源覆盖率、未知项、内容阶段和近期资产；不把 Skill、系统文件或 README 计入资产。
- **待处理**：可直接拖入本地文件；插件只收录原文并建立待处理记录，不自动调用 AI。用户点击“用 `/fde-ingest` 处理”后，才按“原文 → 事实 → 推断 → 未知”生成分流预览。
